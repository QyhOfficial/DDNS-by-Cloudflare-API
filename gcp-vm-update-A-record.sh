...
RECORD_ID=$(curl -s -X GET "https://api.cloudflare.com/client/v4/zones/${ZONE_ID}/dns_records?name=${CLOUDFLARE_RECORD_NAME}.${CLOUDFLARE_ZONE_NAME}" \
     -H "Authorization: Bearer ${CLOUDFLARE_API_TOKEN}" \
     -H "Content-Type: application/json" | jq -r '.result[0].id')

if [ -z "$RECORD_ID" ]; then
    echo "$(date): Error: Failed to find DNS record ID. Please make sure the record exists in Cloudflare." >> "$LOG_FILE"
    exit 1
fi
...
# Get the old IP address from Cloudflare
echo "$(date): Fetching old IP address from Cloudflare..." >> "$LOG_FILE"
